...
export async function suggestTags(plugin: TagSuggestionPlugin, content: string): Promise<string[]> {
    let suggestedTagsJson = "";
    try {
        // Create a chat completion request to OpenAI
        const response = await plugin.openai.chat.completions.create({
            model: plugin.settings.model,
            messages: [
                { role: "system", content: "You should always respond a JSON\nYou are a helpful assistant that suggests relevant tags for a given text content." },
                { role: "user", content: `Suggest 15 relevant tags for the entire following content, return them as a JSON array of strings:\n\n${content}` }
            ],
        ...
        suggestedTagsJson = response.choices[0]?.message?.content?.trim() || "[]";
...
export async function suggestNames(plugin: TagSuggestionPlugin, content: string): Promise<string[]> {
    let suggestedNamesJson = "";
    try {
        // Create a chat completion request to OpenAI
        const response = await plugin.openai.chat.completions.create({
            model: plugin.settings.model,
            messages: [
                { role: "system", content: "You should always respond a JSON\nYou are a helpful assistant that suggests relevant file names for obsidian note files, given text content. (feel free to use space in the naming)" },
                { role: "user", content: `Suggest 5 relevant file names for the entire following content, return them as a JSON array of strings:\n\n${content}` }
            ],
            temperature: plugin.settings.temperature,
            max_tokens: plugin.settings.max_tokens,
        });
        ...
        suggestedNamesJson = response.choices[0]?.message?.content?.trim() || "[]";
        ...
        console.log("suggestedTagsJson: " + suggestedNamesJson);
